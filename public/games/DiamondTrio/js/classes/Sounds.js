...
createjs.Sound.registerSound("/games/DiamondTrio/source/SOUND/"+slotSettings.slotSounds[i], sId);	
...
addEventListener(SLOT_EVENT_SOUNDMPL,function(){currentSound['X_2']=PlaySound("X_2");});

addEventListener(SLOT_EVENT_SHOWINMPL,function(){currentSound['x2_MONEY']=PlaySound("x2_MONEY");});
...
addEventListener(SLOT_EVENT_STARTRESPIN,function(){
	...
	currentSound['RESPIN']=PlaySound("RESPIN");
...
addEventListener("reelStopSnd",function(ev){
	...
	if(ev.detail.reel==5 || slotStateData.respinMode){
	...
	currentSound['FS'].volume=1;	
		...
		currentSound['RESPIN'].paused=true;	

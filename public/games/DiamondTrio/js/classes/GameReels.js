...
this.ChangeScatters=function(sym){
...
var sm=reelStrip[(rPos+i)];	

sm=self.ChangeScatters(sm);	


	
var sAnim=setObj[sm].res;	
...
var sm=servSymbols[rollCount-2];	
...
this.StartRespin=function(){

slotStateData.respinMode=true;		
slotStateData.respinReel=slotSpinResult.respinReel-1;		
	
if(bonusMode){
	
ServerConnect({slotEvent:"freespin",gameCommand:"respin",slotBet:slotStateData.betline,slotLines:slotStateData.lines});		
	
}else{

ServerConnect({slotEvent:"bet",gameCommand:"respin",slotBet:slotStateData.betline,slotLines:slotStateData.lines});		
	
}

objArr[slotSpinResult.respinReel-1].StartRoll();
...
if(slotStateData.respinMode){
objArr[i].StopReel(15,slotSpinResult.reelsSymbols['reel'+(i+1)]);	
}else{
...
if(slotStateData.respinMode){		
objArr[slotStateData.respinReel].ChangeReelCount();
}else{
...
addEventListener(SLOT_EVENT_STARTRESPIN,function(){self.StartRespin();});	
...
bonusMode=false;
if(slotSpinResult.reelsSymbols['reel1']!=undefined){	
objArr[0].FillServerReel(slotSpinResult.reelsSymbols['reel1'],slotSpinResult.reelsSymbols['rp'][0]);	
objArr[1].FillServerReel(slotSpinResult.reelsSymbols['reel2'],slotSpinResult.reelsSymbols['rp'][1]);	
objArr[2].FillServerReel(slotSpinResult.reelsSymbols['reel3'],slotSpinResult.reelsSymbols['rp'][2]);	
objArr[3].FillServerReel(slotSpinResult.reelsSymbols['reel4'],slotSpinResult.reelsSymbols['rp'][3]);		

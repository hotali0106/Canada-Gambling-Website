...
createjs.Sound.registerSound("/games/BookOfRaCL/source/SOUND/"+slotSettings.slotSounds[i], sId);	
...
currentSound['FS'].volume=0;	
	...
	if((cSym=="P_1" && ev.detail.count>=2) || (cSym=="P_2" && ev.detail.count>=3) || (cSym=="P_3" && ev.detail.count>=3) || (cSym=="P_4" && ev.detail.count>=3) ){
	...
	switch(cSym){
		...
		currentSound['P_1_3']=PlaySound("P_1_3");	
		delayTime=currentSound['P_1_3'].duration;	
		...
		currentSound['P_2_3']=PlaySound("P_2_3");	
		delayTime=currentSound['P_2_3'].duration;	
		...
		currentSound['P_3_3']=PlaySound("P_3_3");	
		delayTime=currentSound['P_3_3'].duration;	
		...
		currentSound['P_4_3']=PlaySound("P_4_3");	
		delayTime=currentSound['P_4_3'].duration;	
...
addEventListener(SLOT_EVENT_OPENBOOK,function(){PlaySound("PAGE");});

addEventListener(SLOT_EVENT_BOOKSYM,function(){PlaySound("TICK");});

addEventListener(SLOT_EVENT_BOOKSYMADD,function(){PlaySound("CHING");});

addEventListener(SLOT_EVENT_EXPWINLINE,function(){PlaySound("DING");});

addEventListener(SLOT_EVENT_BOOKSYMADD,function(){PlaySound("CHING");});

addEventListener(SLOT_EVENT_BOOKTOHEADER,function(){PlaySound("BOOK");});

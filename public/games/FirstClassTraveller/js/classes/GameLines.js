...
var cPos=parseInt(slotSpinResult.winLines[i]['winReel'+j][0]);	
	...
	var chSym=ChangeSymFS(cSym);	
		
	var cprop=libArr['lnWinSym_'+chSym];	
		
	winArr[j][cPos]=new WinSymbol(this._view,cprop.GetAnimData(),chSym,cprop.GetSprite());
...
var winEvent=new CustomEvent(SLOT_EVENT_WINLINE,{detail:{stepwin:slotSpinResult.winLines[currentWinLine].stepWin,win:slotSpinResult.winLines[currentWinLine].Win,line:slotSpinResult.winLines[currentWinLine].Line,count:slotSpinResult.winLines[currentWinLine].Count,wcount:slotSpinResult.winLines[currentWinLine].WildCount}});		
dispatchEvent(winEvent);	
...
/*----------отображение вилдов на линии-------------*/	
this.ShowWildLine=function(){

	
slotStateData.wildCount=1;
slotStateData.wildXCount=2;
slotStateData.wildInterval=0;
...
slotStateData.wildInterval=setInterval(function(){	
	
var rPos=gameReels.GetPositions();	

if(slotStateData.wildCount>5){

clearInterval(slotStateData.wildInterval);
if(slotStateData.wildArr!=undefined){	
slotStateData.wildArr.StopAnim();	
delete slotStateData.wildArr;	
}
...
for(var j=slotStateData.wildCount;j<=5;j++){



	
	var cPos=slotSpinResult.winLines[currentWinLine-1]['winReel'+j][0];	


	
	if(cPos!="none" && slotSpinResult.winLines[currentWinLine-1]['winReel'+j][1]=="P_1_0_X"){
	
 dispatchEvent(new Event(SLOT_EVENT_WILDMPL));
	   
var cSym="P_1_"+slotStateData.wildXCount+"_X";
...
var cprop=libArr['lnWinSym_'+chSym];	
	...
	slotStateData.wildArr=new WinSymbol(self._view,cprop.GetAnimData(),cSym,cprop.GetSprite());
		
	slotStateData.wildArr.StartAnim({x:rPos[j-1].x,y:rPos[j-1].y+(symHeight*cPos)});		

	slotStateData.wildXCount=slotStateData.wildXCount*2;	
	slotStateData.wildCount++;  	
...
addEventListener(SLOT_EVENT_WILDLINENEXT,function(ev){timeoutShow=setTimeout(self.ShowWildLine,ev.detail.delay);});		

...
createjs.Sound.registerSound("/games/FirstClassTraveller/source/SOUND/"+slotSettings.slotSounds[i], sId);	
...
tm=0;
	...
	if(ev.detail.wcount>0){
	
	var winSoundEvent=new CustomEvent(SLOT_EVENT_WILDLINENEXT ,{detail:{delay:Math.round(delayTime)}});		
...
addEventListener(SLOT_EVENT_WILDMPL,function(){currentSound['WILD_MPL']=PlaySound("WILD_MPL");});

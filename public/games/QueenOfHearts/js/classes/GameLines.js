...
shape.graphics.beginFill("#ff0000").drawRect(55, 36, 640, 480);
...
}else if(cObj.type=="anim_sprite_lib"){	
...
slotStateData.flyObj=slotSpinResult.flyObj;	
	
slotStateData.flyCount=0;
slotStateData.flyCountTotal=slotStateData.flyObj.length;//общее число подставляемых вилдов 
...
this.StartFlyStep=function(){

var cprop=libArr['AngelAnim'];	
objArr['angel']=new AnimSymbol(self._view,cprop.GetAnimData(),cprop.GetSprite(),"repeate");	
objArr['angel'].StartAnim(cprop.GetSetData());		
objArr['angel'].ChangeState("anim","play");	
var rPos=gameReels.GetPositions();		
	

self.rCnt=slotStateData.flyObj[slotStateData.flyCount][0];	
self.sCnt=slotStateData.flyObj[slotStateData.flyCount][1];	
	
objArr['angel'].GetSprite().x=600+(self.rCnt*70);	
objArr['angel'].GetSprite().y=20;	
createjs.Tween.get(objArr['angel'].GetSprite()).to({y:50,x:rPos[self.rCnt].x-110}, 2000 ).to({y:rPos[self.rCnt].y-30+(self.sCnt*100)}, self.sCnt*500).call(self.FreeFly);		
	
};

this.NextFly=function(){


slotStateData.flyCount++;	
if(slotStateData.flyCount>slotStateData.flyCountTotal-1){
...
dispatchEvent(new Event(SLOT_EVENT_STOPFLY));		
...
this.FreeFly=function(){

slotSpinResult.reelsSymbols['reel'+(self.rCnt+1)][self.sCnt]=slotSettings.wild;	

dispatchEvent(new Event(SLOT_EVENT_RESETSYM));	
	
var sObj={x:objArr['angel'].GetSprite().x,y:objArr['angel'].GetSprite().y};

objArr['angel'].StopAnim();	
	
var cprop=libArr['AngelAnimA'];	
objArr['angel']=new AnimSymbol(self._view,cprop.GetAnimData(),cprop.GetSprite(),"repeate");	
objArr['angel'].StartAnim(cprop.GetSetData());		
objArr['angel'].ChangeState("anim","play");		
	
objArr['angel'].GetSprite().x=sObj.x+30;	
objArr['angel'].GetSprite().y=sObj.y;	
	
createjs.Tween.get(objArr['angel'].GetSprite()).to({y:650}, 3000-((self.sCnt*400)) ).call(self.NextFly);	
...
if(isMainScatters){	
slotStateData.scattersCount=scCount;	
...
addEventListener(SLOT_EVENT_STARTFLY,function(){self.StartFly();});

...
var SLOT_EVENT_SHOWINMPL="SHOWINMPL";
var SLOT_EVENT_SHOWINMPLANIM="SHOWINMPLANIM";
...
addEventListener("reelStop",function(ev){
	...
	if(bonusMode){
	var rCnt=slotSettings.slotReelsConfig.length-slotSettings.slotBonusReelsFixed.countFixedReels;
	}else{
	var rCnt=slotSettings.slotReelsConfig.length;
	}
	
	if(ev.detail.reel==rCnt){
		...
		if(bonusMode){
			setTimeout(function(){dispatchEvent(new Event(SLOT_EVENT_AFTERSPIN));	},400);	
		}else{
		dispatchEvent(new Event(SLOT_EVENT_AFTERSPIN));		
		}
...
addEventListener(SLOT_EVENT_AFTERSPIN,function(){
	...
	if(slotSpinResult.freeMultiplierStatic!=undefined){

	
	gameLines.ShowMultiplierSym();		
	
		
    
	
	setTimeout(gameLines.RemoveMultiplierSym,800);	
    ...
    gameLines.ShowMultiplier();	
		
	}else{	
		
	setTimeout(function(){dispatchEvent(new Event(SLOT_EVENT_ENDBONUS));},500);	
	...
	setTimeout(function(){dispatchEvent(new Event(SLOT_EVENT_SPINBONUS));},10);	

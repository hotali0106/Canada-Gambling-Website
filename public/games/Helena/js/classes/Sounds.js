...
createjs.Sound.registerSound("/games/Helena/source/SOUND/"+slotSettings.slotSounds[i], sId);	
...
currentSound['FS'].volume=0;	
	...
	if(((cSym=="P_1" && ev.detail.count>=2) || (cSym=="P_2" && ev.detail.count>=3) || (cSym=="P_3" && ev.detail.count>=3) || (cSym=="P_4" && ev.detail.count>=3) || (cSym=="P_5" && ev.detail.count>=3) || (cSym=="P_6" && ev.detail.count>=3)) && advancedSound){
...
addEventListener(SLOT_EVENT_STARTSPIN,function(){
	...
	currentSound['REELSET_GO']=PlaySound("REELSET_GO");

	}else if(bonusMode && currentSound['FS']!=undefined && soundMode){
	
	  
	if((slotSpinResult.currentFreeGames+1)==4){
	 
	currentSound['FS'].paused=true;  
	  
	currentSound['FS']=PlaySound("HEL_feature_reelspin_2", {loop:-1});
        PlaySound('HEL_feature_pitchbend_2'); 
	  
	}else if((slotSpinResult.currentFreeGames+1)==8){
	
	 currentSound['FS'].paused=true;   
	  
	currentSound['FS']=PlaySound("HEL_feature_reelspin_3", {loop:-1});
        PlaySound('HEL_feature_pitchbend_3');   
	  
	}else if((slotSpinResult.currentFreeGames+1)==12){
	
	currentSound['FS'].paused=true;    
	  
	currentSound['FS']=PlaySound("HEL_feature_reelspin_4", {loop:-1});
        PlaySound('HEL_feature_pitchbend_4');   
...
addEventListener(SLOT_EVENT_FIRSTSPINBONUS,function(){
 ...
 currentSound['FS']=PlaySound("HEL_feature_reelspin_1", {loop:-1});
 PlaySound('HEL_feature_pitchbend_1');
...
addEventListener("reelStopSnd",function(ev){
	...
	}else if(ev.detail.teaser2){
	
		
		
	currentSound["P_1_TEASER"]=PlaySound("P_1_TEASER");

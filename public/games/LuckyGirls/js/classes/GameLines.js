...
var cPos=i;	
	...
	var cprop=libArr['lnWinSym_FS'];	
		
	var sett=cprop.GetSetData();
	
	winArr[0][0]=new WinSymbol2(this._view,cprop.GetAnimData(),cSym);
	
    winArr[0][0].StartAnim({x:sett.x,y:sett.y},cprop.GetSetData().advanced_set[0][cprop.GetSetData().advanced_set[0].length-1]);	
...
var cprop=libArr['lnWinSym_'+cSym];		
winArr[0][0].StopAnim();	
winArr[0][0]=new WinSymbol2(self._view,cprop.GetAnimData(),cSym);
	
var sFrame=cprop.GetSetData().advanced_set[1][0];	
var tFrame=cprop.GetSetData().advanced_set[1][1];	
totalSym++;	
winArr[0][0].StartAnimDelay({x:cprop.GetSetData().x,y:cprop.GetSetData().y},tFrame,sFrame,totalSym);		

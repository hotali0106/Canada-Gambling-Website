...
createjs.Sound.registerSound("/games/JustJewelsDX/source/SOUND/"+slotSettings.slotSounds[i], sId);	
...
currentSound['FS'].volume=0;	
	...
	if(ev.detail.lcounter<ev.detail.alllines){
	  
	currentSound['jj_short_'+sIdQ[ev.detail.symbol]+'_'+ev.detail.lcounter]=PlaySound('jj_short_'+sIdQ[ev.detail.symbol]+'_'+ev.detail.lcounter);	
	delayTime=currentSound['jj_short_'+sIdQ[ev.detail.symbol]+'_'+ev.detail.lcounter].duration;		
...
dispatchEvent(winSoundEvent);	
	  ...
	  currentSound['jj_full_'+sIdQ[ev.detail.symbol]]=PlaySound('jj_full_'+sIdQ[ev.detail.symbol]);	
	var delayTime2=currentSound['jj_full_'+sIdQ[ev.detail.symbol]].duration; 
	var winSoundEvent=new CustomEvent(SLOT_EVENT_WINLINENEXT ,{detail:{delay:Math.round(delayTime2)}});		
dispatchEvent(winSoundEvent);	

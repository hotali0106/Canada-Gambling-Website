...
var sc=self.ChangeScatterByReel(reelId,reelStrip[(rPos+i)]);		
	
var sAnim=setObj[sc].res;	
...
var sc=self.ChangeScatterByReel(reelId,servSymbols[i-1]);		
...
var sAnim1=reelStrip[curPosition];	

	

	
sAnim1=self.ChangeScatterByReel(reelId,sAnim1);

var sAnim=setObj[sAnim1].res;
...
var sc=self.ChangeScatterByReel(reelId,reelStrip[curPosition]);		
if(rollCount<=6){

var chSym1=self.GetRandomSym(sc);		
...
var sc=self.ChangeScatterByReel(reelId,servSymbols[rollCount-2]);	
...
if(slotSettings.slotBonusReelsFixed['reel'+(i+1)]!=undefined && bonusMode){
